...
// POST /api/admin/logs/parse - Parse log text
router.post('/logs/parse', authenticateAdmin, (req, res) => {
    try {
        const { logText, filters } = req.body;
        ...
        // Parse logs
        const logs = LogParser.parseLogLines(logText);

        // Filter if filters provided
        const filteredLogs = filters ? LogParser.filterLogs(logs, filters) : logs;

        // Analyze logs
        const stats = LogParser.analyzeLogs(filteredLogs);
        ...
        res.status(500).json({
            success: false,
            error: 'Error parsing logs: ' + error.message
        });
    }
});

// GET /api/admin/logs/stats - Get statistics from parsed logs
router.post('/logs/stats', authenticateAdmin, (req, res) => {
    try {
        const { logText, filters } = req.body;
        ...
        res.status(500).json({
            success: false,
            error: 'Error analyzing logs: ' + error.message
        });
    }
});
